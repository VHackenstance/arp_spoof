...
def enable_port_fowarding():
    ...
    # check if input is valid for yes
...
def check_port_forwarding():
    # enable port forwarding: echo 1 | sudo tee /proc/sys/net/ipv4/ip_forward
    # check if port forwarding enabled: cat /proc/sys/net/ipv4/ip_forward  # should print 1
    try:
        ...
        if value == "1":
            ...
            print("[-] Port Forwarding disabled. We need to enable to proceed.")
            exit()
        else:
